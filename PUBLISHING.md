...
- The release merged manifest is checked by CI for package ID, version, target SDK, and permissions. Its only permission is AndroidX's app-scoped `DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION`, required to protect non-exported dynamic receivers and limited to the app's own signature. No INTERNET or other runtime permissions are expected. CI also checks arm64/x86_64 native libraries, 16 KB ELF alignment, and APK zip alignment from the generated AAB.
