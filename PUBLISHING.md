...
- Android compile and target SDK follow the Flutter SDK defaults. CI pins Flutter 3.47.5, whose Android defaults are API 36.
- The main manifest requests no device permissions. Debug and profile manifests include INTERNET for Flutter tooling; verify the release merged manifest when building.
...
- The repo CI workflow runs `flutter analyze`, `flutter test`, a web release build, and an Android debug APK build. The local environment used for this pass has no Flutter, Dart, Java, or Android SDK executable, so those checks still need a successful CI run before this is a release candidate.
...
2. Obtain the real upload keystore for this app, populate `android/key.properties`, and build/sign `build/app/outputs/bundle/release/app-release.aab`. Do not generate a replacement key if the package already has an upload key.
