...
void main() {
  test('formats timer at minute and hour boundaries', () {
    ...
    expect(
      formatSessionDuration(const Duration(minutes: 59, seconds: 59)),
      '59:59',
    );
  ...
  test(
    'elapsed time can be calculated deterministically and clamps future starts',
    () {
      ...
      expect(
        elapsedSince(now.add(const Duration(seconds: 5)), now: now),
        Duration.zero,
      );
    },
  );
